...
public class PrismsListMessage : NetworkMessage
{
    ...
    public override void Serialize(IDataWriter writer)
    {
        ...
        {
            writer.WriteShort((short)(prisms[_i1] as PrismGeolocalizedInformation).TypeId);
            (prisms[_i1] as PrismGeolocalizedInformation).Serialize(writer);
        }
    ...
    {
        uint _id1 = 0;
        PrismGeolocalizedInformation _item1 = null;
        uint _prismsLen = (uint)reader.ReadUShort();
        for (uint _i1 = 0; _i1 < _prismsLen; _i1++)
        {
            _id1 = (uint)reader.ReadUShort();
            _item1 = ProtocolTypeManager.GetInstance<PrismGeolocalizedInformation>((short)_id1);
