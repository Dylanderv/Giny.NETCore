...
{
    public new const ushort Id = 2084;
    ...
    public RankInformation(int order, int gfxId, bool modifiable, int[] rights, int id, string name)
    {
        this.order = order;
        this.gfxId = gfxId;
    ...
    public override void Deserialize(IDataReader reader)
    {
        ...
        if (order < 0)
        {
            throw new System.Exception("Forbidden value (" + order + ") on element of RankInformation.order.");
        }

        gfxId = (int)reader.ReadVarUhInt();
        if (gfxId < 0)
        {
            throw new System.Exception("Forbidden value (" + gfxId + ") on element of RankInformation.gfxId.");
        }
