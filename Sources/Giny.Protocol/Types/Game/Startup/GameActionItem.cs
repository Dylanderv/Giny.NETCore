...
public class GameActionItem
{
    ...
    public GameActionItem(int uid, string title, string text, string descUrl, string pictureUrl, ObjectItemInformationWithQuantity[] items)
    {
        this.uid = uid;
        this.title = title;
        this.text = text;
    ...
    public virtual void Deserialize(IDataReader reader)
    {
        ...
        if (uid < 0)
        {
            throw new System.Exception("Forbidden value (" + uid + ") on element of GameActionItem.uid.");
        }
