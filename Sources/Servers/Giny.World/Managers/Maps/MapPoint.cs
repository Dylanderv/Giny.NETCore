...
public class MapPoint
{
    ...
    public short DistanceTo(MapPoint point)
    {
        return (short)System.Math.Sqrt((double)((point.X - this.m_x) * (point.X - this.m_x) + (point.Y - this.m_y) * (point.Y - this.m_y)));
    }
    public short ManhattanDistanceTo(MapPoint point)
    {
        return (short)(Math.Abs(m_x - point.X) + Math.Abs(m_y - point.Y));
    }

    public short SquareDistanceTo(MapPoint point)
    {
        return (short)Math.Max(Math.Abs(m_x - point.X), Math.Abs(m_y - point.Y));
    }

    public bool IsAdjacentTo(MapPoint point)
    {
        return this.ManhattanDistanceTo(point) == 1u;
    }
    ...
    public static short CoordToCellId(int x, int y)
    {
        ...
        return (short)((x - y) * MapWidth + y + ((x - y) / 2));
    }
